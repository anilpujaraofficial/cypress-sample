...
context("Investigation > Histology", () => {
  before(() => {
  ...
  beforeEach(() => {
    cy.adminLogin();
    cy.intercept(getEnvVariables("apiUrl") + "/labs/patient-list/HISTO").as(
      "data"
    );
    navigation(["Investigation", "Histology"]);
  ...
  describe("Patient List || Sample Collection || Lab Receive || Department Receive", () => {
    ...
    it("Sample Collection", () => {
      histoObj.collectSample(filePath.bill, filePath.patient, "HISTO");
    });
    it("Lab Receive", () => {
  ...
  //   });
  //   it("Cancel Test", () => {
  //     histoObj.testCancel(filePath.bill, filePath.patient, "HISTO");
  //   });
  // });
});
